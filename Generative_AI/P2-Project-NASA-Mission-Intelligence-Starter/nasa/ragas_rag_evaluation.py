...
class RAGSystemEvaluator:
    ...
    def __init__(self, openai_api_key: str = None, base_url: str = None):
        """
        Initialize the RAG system evaluator.

        Args:
            openai_api_key (str): OpenAI API key for LLM-based evaluations
            base_url (str): Base URL for OpenAI API (for Vocareum or other custom endpoints)
        """
        # Use Vocareum defaults if not provided
        if openai_api_key is None:
            openai_api_key = "voc-341242580126677498268469e38160d05be0.48783907"
        if base_url is None:
            base_url = "https://openai.vocareum.com/v1"

        self.openai_client = OpenAI(api_key=openai_api_key, base_url=base_url)
        self.evaluator_llm = LangchainLLMWrapper(ChatOpenAI(model="gpt-4o", api_key=openai_api_key, base_url=base_url))
        langchain_openai_embeddings = OpenAIEmbeddings(openai_api_key=openai_api_key, base_url=base_url)
    ...
    def evaluate_rag_system(self, dataset: Dataset, config_key: str = "comprehensive") -> Dict:
        ...
        try:
            ...
            # Extract metric scores - handle different RAGAS result formats
            total_score = 0
            metric_count = 0

            # Try to get results as pandas DataFrame
            results_df = results.to_pandas() if hasattr(results, 'to_pandas') else None
            formatted_results["detailed_scores"] = results_df

            if results_df is not None and not results_df.empty:
                # Extract column means for each metric (average across all samples)
                for metric in config["metrics"]:
                    metric_name = metric.name
                    if metric_name in results_df.columns:
                        # Get mean score across all samples, ignoring NaN
                        score = results_df[metric_name].dropna().mean()
                        if not np.isnan(score):
                            formatted_results["metrics"][metric_name] = round(float(score), 4)
                            total_score += score
                            metric_count += 1
            else:
                # Fallback: try accessing results directly as dict-like
                for metric in config["metrics"]:
                    metric_name = metric.name
                    try:
                        if hasattr(results, 'scores') and metric_name in results.scores:
...
def demonstrate_ragas_evaluation():
    ...
    # Initialize evaluator with Vocareum API
